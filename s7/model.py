...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1_dilated = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=64, kernel_size=(3,3), padding=2, dilation=2, bias=False),
                                   nn.ReLU(),
                                  nn.BatchNorm2d(64),
                                  nn.Dropout2d(dropout_value))# Input=32x32x3 Output=32x32x16 RF=7x7
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(3,3), padding=1, bias=False),
                                   nn.ReLU(),
                                  nn.BatchNorm2d(128),
                                  nn.Dropout2d(dropout_value))# Input=16x16x16 Output=16x16x32 RF=12x12
